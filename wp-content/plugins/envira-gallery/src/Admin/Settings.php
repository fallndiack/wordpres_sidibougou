...
class Settings {
	...
	public function standalone_settings_save() {
		...
		foreach ( $slugs_to_check as $slug ) {
			...
			if ( 'albums' !== $slug && ( strtolower( wp_unslash( $_POST['envira-albums-slug'] ) ) === strtolower( $_POST[ 'envira-' . $slug . '-slug' ] ) ) ) { // @codingStandardsIgnoreLine
	...
	public function settings_page() {
		...
		<h2 id="envira-tabs-nav" class="envira-tabs-nav" data-container="#envira-gallery-settings" data-update-hashbang="1">
			...
			foreach ( (array) $this->get_envira_settings_tab_nav() as $id => $title ) {
				...
				?>
				<a class="nav-tab <?php echo esc_html( $class ); ?>" href="#envira-tab-<?php echo esc_attr( $id ); ?>" title="<?php echo esc_attr( $title ); ?>"><?php echo esc_html( $title ); ?></a>
				<?php
				$i++;
			}
			?>
			...
			<table class="form-table">
				<tbody>
					<tr id="envira-settings-key-box">
						...
						<td>
							<form id="envira-settings-verify-key" method="post">
								...
								<p class="description"><?php esc_html_e( 'License key to enable automatic updates for ', 'envira-gallery' ); ?><?php echo esc_html( apply_filters( 'envira_whitelabel_name', 'Envira' ) ); ?></p>
							</form>
						</td>
					</tr>
					...
					<tr id="envira-settings-key-type-box">
						<th scope="row">
							<label for="envira-settings-key-type"><?php echo esc_html( apply_filters( 'envira_whitelabel_name', 'Envira' ) ); ?><?php esc_html_e( ' License Key Type', 'envira-gallery' ); ?></label>
						</th>
						<td>
							<form id="envira-settings-key-type" method="post">
								<?php /* translators: %s: license key */ ?>
								<span class="envira-license-type"><?php esc_html_e( 'Your license key type for this site is ', 'envira-gallery' ); ?><strong><?php echo esc_attr( envira_get_license_key_type() ); ?>.</strong>
					...
					<form action="edit.php?post_type=envira&page=envira-gallery-settings#!envira-tab-standalone" method="post">
						...
						<tr id="envira-settings-slug-box-gallery">
							...
							<td>
								...
								<p class="description"><?php esc_html_e( 'The slug to prefix to all ' . apply_filters( 'envira_whitelabel_name', 'Envira' ) . ' Galleries.', 'envira-standalone' ); // @codingStandardsIgnoreLine?></p>
							</td>
						</tr>
						...
						<tr id="envira-settings-slug-box-albums">
							...
							<td>
								...
								<p class="description"><?php esc_html_e( 'The slug to prefix to all ' . apply_filters( 'envira_whitelabel_name', 'Envira' ) . ' Albums.', 'envira-standalone' ); // @codingStandardsIgnoreLine?></p>
							</td>
						</tr>
