...
function envirabox_default_template( $data ) {
	...
	if ( ! empty( $data['config']['lightbox_title_caption'] ) && '0' !== $data['config']['lightbox_title_caption'] ) {
		$template .= ( isset( $data['config']['lightbox_title_caption'] ) && 'title' === $data['config']['lightbox_title_caption'] ) ? '<div class="envirabox-title envirabox-title-item-id-' . $data['id'] . '' . ( envira_get_config( 'image_counter', $data ) ? ' with-counter' : false ) . '"></div>' : '<div class="envirabox-caption envirabox-caption-item-id-' . $data['id'] . '' . ( envira_get_config( 'image_counter', $data ) ? ' with-counter' : false ) . '"></div>';
	}
	$template .= '</div">';
...
function envira_load_lightbox_config( $gallery_id, $raw = false, $gallery_type = false ) {
	...
	if ( intval( envira_get_config( 'supersize', $data ) ) === 1 ) {
		...
		$config['margins'] = ( in_array( envira_get_config( 'lightbox_theme', $data ), $legacy_themes, true ) && ( null !== envira_get_config( 'lightbox_title_caption', $data ) ) ) ? array( 100, 0 ) : array( 10, 0 );
